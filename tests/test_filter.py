...
from src.filter import get_statistic, make_transactions, normalize_transaction, search_transactions
...
def test_make_transactions2() -> None:
    ...
    with patch("builtins.input", return_value="2"), patch("src.filter.get_transactions_csv") as mock_get_ops:
        mock_get_ops.return_value = [{"id": 1}, {"id": 2}]
        result = make_transactions()
        assert result[1] == {"id": 2}
    mock_get_ops.assert_called_once()
